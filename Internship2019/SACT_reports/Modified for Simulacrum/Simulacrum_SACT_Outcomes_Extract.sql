...
/*  This means that the 'Provider/Trust that initiated treatment' may change between regimens for the same Merged_Tumour_ID where they would not in the real SACT data */
	...
	MAX(NVL(SIM_SACT_C.Start_Date_of_Cycle, '')) KEEP (DENSE_RANK LAST ORDER BY SIM_SACT_C.Start_Date_of_Cycle, SIM_SACT_C.Merged_Cycle_ID) AS Latest_Cycle,
	MAX(NVL(SIM_SACT_D.Administration_Date, '')) KEEP (DENSE_RANK LAST ORDER BY SIM_SACT_D.Administration_Date, SIM_SACT_D.Merged_Drug_Detail_ID) AS Latest_Admin_Date
